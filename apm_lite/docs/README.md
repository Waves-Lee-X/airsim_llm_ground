...
| 09 | [相机视频与视觉语义](09-相机视频与视觉语义.md) | D435i、RTSP、OpenCV、延迟、VLM 和当前边界 | RGB/深度已联调（AirSim） |
| 10 | [Mission Agent 设计与调试](10-Mission-Agent设计与调试.md) | 多轮对话、实时状态、工具草案、二次门禁 | Agent 端到端闭环已验收（仿真） |
...
| 15 | [测试验收与后续开发](15-测试验收与后续开发.md) | 334 项测试、L0-L6 分级调试和后续 M5-M8 | 当前验收入口 |
| 16 | [编队设计与验收](16-编队设计与验收.md) | 三种队形、平滑切换、槽位分配、时空碰撞与失联策略 | 已验收（Web 执行，误差≤0.06m） |
| 17 | [视觉目标导航与深度停靠](17-视觉目标导航与深度停靠.md) | VLM+深度定位、搜索/对准、standoff、颜色共识 | 已验收（仿真） |
| 18 | [M1 多保真平行推演与证据包](18-M1多保真平行推演与证据包.md) | 统一孪生契约、L0 分支、Pareto 和证据包 | 已实现（L0） |
| 19 | [无人机身份文档](19-无人机身份文档.md) | 签名身份、能力、机载模型、授权和注册表 | P0 已实现 |
...
| 94 | [M5 四机编队仿真验收记录](94-M5四机编队仿真验收记录.md) | 三队形各 10 轮回归，最小间距≥2.75 m |
| 95 | [视觉目标导航与 Agent 闭环仿真验收记录](95-视觉目标导航与Agent闭环仿真验收记录.md) | 深度停靠 5.05/5.92 m、Agent 闭环、编队 Web 执行、颜色共识 |
