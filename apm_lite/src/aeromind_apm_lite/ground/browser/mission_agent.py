...
class MissionAgentService:
    ...
    _SYSTEM_PROMPT = (
        ...
        "且只能指定一架飞机。"
        "用户要求‘向目标方向飞 X 米’时应使用 analyze 的 navigate_after=true 与 "
        "approach_distance_m=X；要求‘停在目标外侧（表面）前方 X 米’时使用 "
        "navigate_after=true 与 standoff_distance_m=X（规划器通过深度传感器定位目标表面，"
        "停在目标外侧 X 米、机头朝向目标）。不要凭空编造 goto 坐标；当 analyze 已设置 "
        "navigate_after 时，不要再额外生成 goto 步骤。"
            ...
            if action == "takeoff":
                ...
                if navigate is not None and not isinstance(navigate, bool):
                    if isinstance(navigate, dict):
                        # The model sometimes emits a hint object (e.g. a
                        # target position); the boolean alone is what the
                        # planner needs, so normalize it to True.
