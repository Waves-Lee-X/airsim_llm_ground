...
def fleet_launch_config(
    vehicle_count: int = 4,
    *,
    spacing_m: float = 3.0,
    state_directory: Path | None = None,
    airsim_settings_path: Path | None = None,
    wsl_ip: str | None = None,
) -> SimulationLaunchConfig:
    """Build a deterministic multi-vehicle SITL launch configuration.

    Vehicles are spread in a horizontal line at the simulated home so they
    start safely separated; formation slot offsets are generated by
    :func:`aeromind_apm_lite.common.formation.formations.formation_offsets`.
    """
    if not 2 <= vehicle_count <= 8:
        raise SimulationConfigError("fleet launch requires 2 to 8 vehicles")
    if spacing_m <= 0.0:
        raise SimulationConfigError("fleet spacing must be positive")
    ...
    instances = tuple(
        SitlInstanceConfig(
            instance=index,
            vehicle_name=f"Drone{index + 1}",
            initial_pose=InitialPoseNed(
                x_m=offset_x,
                y_m=offset_y,
                z_m=-0.5,
            ),
    ...
    default_state = Path.home() / "aeromind-sitl" / f"fleet-{vehicle_count}"
    default_settings = (
        Path.home() / "aeromind-sitl" / f"fleet-{vehicle_count}" / "settings.json"
    )
    return SimulationLaunchConfig(
        instances=instances,
        paths=SimulationPaths(
            state_directory=state_directory or default_state,
            airsim_settings_path=airsim_settings_path or default_settings,
        ),
        wsl_ip=wsl_ip or discover_wsl_ipv4(),
    )
