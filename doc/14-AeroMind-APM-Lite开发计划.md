...
- 每架树莓派 4B 只运行轻量机载代理、`pymavlink`、D435 采集、OpenCV 和本机安全逻辑。
- APM 运行 ArduCopter，负责姿态稳定、电机控制、EKF、GUIDED、LAND 和 RTL。
...
| 路径规划与避障 | 仿真建筑、街道、球门和隧道生成可执行路径；真实场地按统一坐标摆放纸箱和球门；D435 提供本机安全兜底 |
...
4. **本机安全闭环**：失联、目标过期、D435 近障碍和飞控异常必须在树莓派或 APM 本地收敛。
...
M0 必须锁定 ArduCopter 版本或代码提交、机架类型、参数文件、AirSim 配置、四机 `SYSID_THISMAV` 和 UDP 端口矩阵。任务执行只能经过 `onboard_agent -> MAVLink -> SITL -> AirSim` 闭环；AirSim RPC 仅用于传感器读取、场景管理和镜像实体显示。
...
仿真 `AirSimSensorSource`、真机 `RealSenseSensorSource` 和离线 `ReplaySensorSource` 必须输出同一 RGB/Depth 契约，使 D435 安全算法可以使用相同录包完成虚实回归。
...
VLM 结果标记为模型推断，必须保留原图、提示词、模型、响应和时间。VLM 超时或失败不能阻塞飞控遥测、D435 安全检查或任务取消。
...
5. 首版 D435 安全层只实现“限速、HOLD、取消当前段”，定义为防碰撞安全停止，不宣称已经具备自主绕障。
...
1. 地面为每架无人机分配箱体和观察航点。
2. 到达后 HOLD，采集多帧 RGB 和对应深度、位姿。
3. OpenCV 和 VLM 生成编号、颜色、所在面、距离、置信度和证据。
4. 数字孪生更新箱体语义属性，任务管理器完成颜色目标重新分配。
5. 规划器生成目标面附近的安全观察点和视觉引导降落段。
6. 末端定位依赖视觉标志/相对定位，不依赖普通 GPS 直接降到箱体附近。

D435 的安装方向和观察机动必须在 M0 冻结。单个固定前视相机无法天然观察箱体前、后、左、右和顶面；顶面任务需要经验证的俯视机动、可调安装，或增加独立下视 RGB 相机。隧道若造成 GPS 拒止，必须增加 AprilTag/UWB/VIO 等定位来源；没有定位来源时只能把隧道限制为保持 GPS 可用的演示结构，不能宣称完成 GPS 拒止自主穿越。
...
1. 评审并冻结本文档中的模块边界、运行模式和首个垂直切片。
2. 明确四套 APM、树莓派、D435、数传、遥控器和地面网络的实际型号与连接方式。
3. 解决“需求描述中先写四个箱体、后写五个箱体”的数量冲突，并定义“离颜色标识面最近位置”的安全降落点、朝向和允许误差。
4. 冻结 D435 安装方式、顶面观察方案和隧道定位边界。
5. 建立 `apm_lite/` 骨架和 Python 依赖管理。
6. 定义 v1 JSON Schema、认证机制、任务状态机和错误码。
7. 实现假飞控测试端，再接单机 ArduPilot SITL。
8. 为旧 `swarmxian260413.py` 建立“可复用逻辑/必须重写逻辑”迁移清单。
9. 盘点现有 AeroMind 的 LLM、VLM、Workflow 和 Web 代码，确定服务化复用边界。

## 18. M0 前必须确认的外部条件

以下问题会改变实现方案，进入 M1 前必须形成书面结论：

1. 实际是四个还是五个箱体，每架无人机与箱体是否一一对应。
2. APM 飞控具体型号、ArduCopter 版本、可用 UART 数量和电气连接方式。
3. 四台树莓派的内存版本、操作系统、供电、散热和 D435 USB 带宽。
4. D435 安装角度，是否允许增加下视相机、云台、AprilTag、RTK、UWB 或 VIO。
